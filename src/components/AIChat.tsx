...
const KNOWLEDGE_BASE: Record<string, string> = {
  greeting:
    "Hello! I'm the Averon Partners assistant. I can tell you about our services, expertise, and how we help technology companies grow in Europe and the US. What would you like to know?",
  services:
    "We offer four core services:\n\n• **B2B Client Acquisition** — full pipeline development from prospecting to closing\n• **SaaS Market Expansion** — go-to-market strategies for European and US entry\n• **Strategic Partnerships** — channel and alliance building across markets\n• **Sales Process Optimization** — improving your sales efficiency and conversion rates\n\nWould you like details on any specific service?",
  about:
    "Averon Partners is a business development firm specializing in helping SaaS, AI, automation, and technology companies acquire B2B clients and expand in European and US markets. We bring years of experience in negotiations, client relationships, and closing deals.",
  industries:
  ...
  europe:
    "We work with companies expanding remotely across Europe and the United States. For companies in Poland, we also support offline market development and direct partnerships. We understand the diverse business cultures and buyer behaviors across these markets.",
  pricing:
  ...
  meeting:
    "Great idea! Click the 'Book a Call' button in the navigation — you can pick a date and time that works for you and we'll send a calendar invitation. We'd love to learn about your growth goals.",
};
...
function getResponse(input: string): string {
  ...
  if (lower.match(/\b(europ|market|country|countries|region|poland|us |united states)\b/)) return KNOWLEDGE_BASE.europe;
  ...
  if (lower.match(/\b(acqui|client|lead|prospect|pipeline)\b/))
    return "Our B2B Client Acquisition service covers the entire pipeline — from identifying ideal prospects to closing deals. We use targeted outreach, relationship-driven selling, and data-backed strategies to build your client base. Would you like to book a call to discuss?";
  if (lower.match(/\b(partner|channel|alliance|reseller)\b/))
    return "We help build strategic partnerships, channel alliances, and reseller networks. This multiplies your market reach without the overhead of direct sales operations. Interested in learning more?";
  if (lower.match(/\b(expansion|expand|enter|go.to.market|gtm)\b/))
    return "Our SaaS Market Expansion service provides comprehensive go-to-market strategies. From market research and competitive analysis to first revenue, we guide the entire journey. Shall we discuss your expansion plans?";
  if (lower.match(/\b(process|optim|sales cycle|conversion)\b/))
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 z-50 w-[360px] max-w-[calc(100vw-3rem)] h-[500px] max-h-[calc(100vh-10rem)] rounded-2xl overflow-hidden shadow-2xl shadow-accent-blue/10 border border-dark-600/50 flex flex-col bg-dark-800"
          >
            <div className="flex items-center gap-3 px-5 py-4 border-b border-dark-600/50 bg-gradient-to-r from-dark-700/50 to-dark-800">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-accent-blue to-accent-purple flex items-center justify-center">
                <Bot size={16} className="text-white" />
              </div>
              <div>
                <div className="text-sm font-semibold text-white">Averon Assistant</div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
              ...
              {isTyping && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-2.5">
                  ...
                  <div className="px-4 py-3 rounded-2xl rounded-tl-md bg-dark-700/60">
            ...
            <form onSubmit={handleSend} className="px-4 py-3 border-t border-dark-600/50 flex gap-2">
              <input
                type="text"
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="Ask about our services..."
                className="flex-1 px-4 py-2.5 rounded-xl text-sm bg-dark-700/60 text-white placeholder:text-dark-400 border border-dark-600/30 focus:outline-none focus:border-accent-blue/50 transition-colors"
              />
              <button
                type="submit"
                disabled={!input.trim()}
